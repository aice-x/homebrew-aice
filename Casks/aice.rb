# This file was generated by GoReleaser. DO NOT EDIT.
cask "aice" do
  require "download_strategy"

  class GitHubPrivateRepositoryReleaseDownloadStrategy < CurlDownloadStrategy
    def initialize(url, name, version, **meta)
      @url = url
      parse_url_pattern
      set_github_token
      meta[:headers] ||= []
      meta[:headers] << "Accept: application/octet-stream"
      meta[:headers] << "Authorization: Bearer #{@github_token}"
      api_url = "https://api.github.com/repos/#{@owner}/#{@repo}/releases/assets/#{asset_id}"
      super(api_url, name, version, **meta)
    end

    def parse_url_pattern
      pattern = %r{https://github\.com/([^/]+)/([^/]+)/releases/download/([^/]+)/(\S+)}
      raise CurlDownloadStrategyError, "Invalid GitHub release URL: #{@url}" unless @url =~ pattern

      _, @owner, @repo, @tag, @filename = *@url.match(pattern)
    end

    private

    def set_github_token
      @github_token = ENV["HOMEBREW_GITHUB_API_TOKEN"]
      @github_token = GitHub::API.credentials if (@github_token.nil? || @github_token.empty?) && defined?(GitHub::API)
      if @github_token.nil? || @github_token.empty?
        raise CurlDownloadStrategyError,
          "No GitHub credentials for the private aice repo. Run `gh auth login` or export HOMEBREW_GITHUB_API_TOKEN (repo scope)."
      end
      GitHub.repository(@owner, @repo)
    end

    def asset_id
      @asset_id ||= begin
        assets = GitHub.get_release(@owner, @repo, @tag)["assets"]
        asset = assets.find { |a| a["name"] == @filename }
        raise CurlDownloadStrategyError, "Asset #{@filename} not found in release #{@tag}" if asset.nil?

        asset["id"]
      end
    end
  end

  version "0.1.6"

  on_macos do
    on_intel do
      sha256 "52d329196cbf5fa1cacec4b4991ec70a6f4403cc55fb9325e502a8eddfa60dd1"
      url "https://github.com/propra-tech/aice/releases/download/v#{version}/aice_#{version}_darwin_amd64.tar.gz",
        verified: "github.com/propra-tech/aice/",
        using: GitHubPrivateRepositoryReleaseDownloadStrategy
    end
    on_arm do
      sha256 "f57ee48b04677c21f9f490886b68ec8a28d138c57e89102b2cd0bc3caf5198a0"
      url "https://github.com/propra-tech/aice/releases/download/v#{version}/aice_#{version}_darwin_arm64.tar.gz",
        verified: "github.com/propra-tech/aice/",
        using: GitHubPrivateRepositoryReleaseDownloadStrategy
    end
  end

  on_linux do
    on_intel do
      sha256 "0e3e7c43dcb309bfc2af7d0b85c499e28735f962c38f8e04833911d6642f6b0b"
      url "https://github.com/propra-tech/aice/releases/download/v#{version}/aice_#{version}_linux_amd64.tar.gz",
        verified: "github.com/propra-tech/aice/",
        using: GitHubPrivateRepositoryReleaseDownloadStrategy
    end
    on_arm do
      sha256 "87a8cc382c1495b392280a75370b563da7399a8817f6a674a736e14770e486de"
      url "https://github.com/propra-tech/aice/releases/download/v#{version}/aice_#{version}_linux_arm64.tar.gz",
        verified: "github.com/propra-tech/aice/",
        using: GitHubPrivateRepositoryReleaseDownloadStrategy
    end
  end

  name "aice"
  desc "Author and manage pages on the Propra intranet (aice)"
  homepage "https://github.com/propra-tech/aice"

  livecheck do
    skip "Auto-generated on release."
  end

  binary "aice"

  postflight do
    if OS.mac?
      system_command "/usr/bin/xattr", args: ["-dr", "com.apple.quarantine", "#{staged_path}/aice"]
    end
  end

  # No zap stanza required

end
